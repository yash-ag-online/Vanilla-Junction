import type { IceCream, Order, ApiResponse } from '@/types';
...
type IceCreamWithId = Omit<IceCream, '_id'> & {
  id: string;
};
...
const IceCreamCard = ({ type, defaultValues }: { type: 'edit' | 'list'; defaultValues?: IceCreamWithId }) => {
  ...
  const api =
    type === 'edit'
      ? `${import.meta.env.VITE_BACKEND_URL}/ice-creams/${defaultValues!.id}`
      : `${import.meta.env.VITE_BACKEND_URL}/ice-creams`;
  ...
  const form = useForm<z.infer<typeof formSchema>>({
    ...
    defaultValues: {
      ...
      discount: defaultValues?.discount || undefined,
  ...
  const onSubmit = async (data: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      if (result.data && result.data.icecream) {
        toast.success(`Ice Cream ${type === 'list' ? 'Listed' : 'Updated'} Successfully.`);
      } else {
        throw new Error('Something went wrong.');
      }
    } catch (error) {
  ...
  return (
    <>
      <DialogHeader>
        <DialogTitle>Ice Cream Details</DialogTitle>
        <DialogDescription>Enter your ice cream detail below to {type}.</DialogDescription>
      </DialogHeader>
          ...
          <Controller
            ...
            render={({ field, fieldState }) => (
              <Field data-invalid={fieldState.invalid}>
                ...
                <Input {...field} id="description" aria-invalid={fieldState.invalid} />
                {fieldState.invalid && <FieldError errors={[fieldState.error]} />}
              </Field>
            )}
          />
      ...
      <DialogFooter className="flex-col gap-2">
        <Button type="submit" form="icecream-form" className="w-full capitalize">
          {loading ? <SpinnerIcon className="animate-spin" /> : type}
        </Button>
      </DialogFooter>
    </>
  );
...
const iceCreamsColumns: ColumnDef<IceCreamWithId>[] = [
  {
    accessorKey: 'name',
    header: ({ column }) => {
      return (
  ...
  },
  {
    ...
    cell: ({ row }) => {
      ...
      return <img src={imageUrl} alt="User avatar" className="size-10" />;
    },
  },
  {
    accessorKey: 'price',
  ...
  },
  {
    id: 'actions',
    cell: ({ row }) => {
      const iceCream = row.original;
      const [deleteLoading, setDeleteLoading] = useState(false);

      const deleteIceCream = async (e: Event) => {
        e.preventDefault();
        setDeleteLoading(true);

        try {
          const response = await fetch(`${import.meta.env.VITE_BACKEND_URL}/ice-creams/${iceCream.id}`, {
            method: 'DELETE',
            credentials: 'include',
          });
              ...
              <DropdownMenuItem onSelect={(e) => e.preventDefault()}>
                <Dialog>
                  ...
                  <DialogContent onKeyDown={(e) => e.stopPropagation()} onOpenAutoFocus={(e) => e.preventDefault()}>
                    <IceCreamCard type={'edit'} defaultValues={iceCream} />
                  </DialogContent>
                </Dialog>
              </DropdownMenuItem>
              <DropdownMenuSeparator />
              <DropdownMenuItem variant="destructive" className="gap-2" onSelect={deleteIceCream}>
                {deleteLoading ? (
...
export const IceCreamsList = () => {
  const { isPending, error, data } = useQuery<ApiResponse<{ icecreams: IceCream[] }>>({
    queryKey: ['iceCreamsData'],
    queryFn: () =>
      fetch(`${import.meta.env.VITE_BACKEND_URL}/ice-creams`, { credentials: 'include' }).then((res) => res.json()),
  });
  ...
  let iceCreamsData: IceCreamWithId[] = [];

  if (data.data?.icecreams?.length > 0) {
    iceCreamsData = data.data.icecreams.map((i: IceCream) => ({
      ...i,
      id: i._id,
    }));
  }
  ...
  return (
    <>
      <div className="w-full flex items-center justify-between gap-2 flex-wrap">
        <h1 className="text-4xl">All Ice Creams</h1>
        <Dialog>
          <DialogTrigger asChild>
            <Button>
              <PlusIcon /> Add Ice Cream
            </Button>
          </DialogTrigger>
          <DialogContent onKeyDown={(e) => e.stopPropagation()} onOpenAutoFocus={(e) => e.preventDefault()}>
            <IceCreamCard type={'list'} />
          </DialogContent>
        </Dialog>
      </div>
      <DataTable columns={iceCreamsColumns} data={iceCreamsData} filterText="name" />
    </>
  );
};
...
export const OrdersList = () => {
  const { isPending, error, data } = useQuery<ApiResponse<{ orders: Order[] }>>({
    queryKey: ['ordersData'],
    queryFn: () =>
      fetch(`${import.meta.env.VITE_BACKEND_URL}/orders?filter=pending`, { credentials: 'include' }).then((res) =>
        res.json(),
      ),
  });
  ...
  return (
    <>
      <h1 className="text-4xl">All Pending Orders</h1>
